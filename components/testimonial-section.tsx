...
export default function TestimonialSlider() {
  ...
  useEffect(() => {
    if (sliderRef.current && logoSliderRef.current) {
      ...
      const logoWidth = 384 + 128; // w-96 (384px) + gap-32 (128px)
      const totalWidth = logoWidth * extendedTestimonials.length;
      const centerIndex = testimonials.length + current; // Use middle set of logos for centering
      const centerOffset = ((centerIndex * logoWidth) / totalWidth) * 100 - (logoWidth * 2 / totalWidth * 100); // Center the current logo
  ...
  return (
    <section className="bg-gray-100 py-10 relative">
      <div className="mx-auto px-24">
        ...
        <button
          onClick={handlePrev}
          className="absolute left-72 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors duration-300 z-10"
        ...
        <button
          onClick={handleNext}
          className="absolute right-72 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors duration-300 z-10"
        ...
        <div className="text-center px-20">
            ...
            <div className="overflow-hidden relative w-full">
              <div ref={sliderRef} className="flex">
                {testimonials.map((testimonial, index) => (
                  ...
                  >
                    <blockquote className="text-lg lg:text-xl leading-none mb-3 px-52 font-thin font-serif italic text-black max-w-4xl">
                      {testimonial.quote}
                    </blockquote>
          ...
          <div className="flex items-center justify-center overflow-hidden">
            <div
              ...
              style={{ minWidth: `${(300) * extendedTestimonials.length}px` }} // w-96 (384px) + gap-32 (128px)
            >
              {extendedTestimonials.map((testimonial, index) => (
                <div key={index} className="flex w-96 items-center justify-center">
